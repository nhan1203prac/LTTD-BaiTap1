...
namespace WebApplication2.Controllers
{
    public class ProductController : ApiController
    {
        ...
        public IHttpActionResult CreateProduct([FromBody] tbSANPHAM newProduct)
        {
            ...
            return CreatedAtRoute("GetProductById", new { id = newProduct.MASANPHAM }, newProduct);
        }

        [HttpPut]
        [Route("api/Product/{id:int}")]
        public IHttpActionResult UpdateProduct(int id, [FromBody] tbSANPHAM updatedProduct)
        {
            if (updatedProduct == null || id != updatedProduct.MASANPHAM)
                return BadRequest("Invalid product data.");

            var existingProduct = db.tbSANPHAMs.Find(id);
